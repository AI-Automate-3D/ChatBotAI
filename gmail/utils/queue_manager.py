"""Queue manager re-export — uses the shared queue_manager from tg.utils.

Gmail's pipeline uses the exact same JSON queue mechanism as the Telegram
pipeline.  This module re-exports the shared functions for convenience so
that Gmail code can import from ``gmail.utils.queue_manager`` without
depending on ``tg``.

If you need to use this outside the ChatBotAI project, copy
``tg/utils/queue_manager.py`` into your project instead.

Usage
-----
    from gmail.utils.queue_manager import load_queue, append_queue, clear_queue
"""
