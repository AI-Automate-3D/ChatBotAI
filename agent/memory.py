"""Standalone conversation memory manager — load, save, clear, and trim history.

Manages conversation history as a JSON file containing a list of
OpenAI-style message dicts (``{"role": "user"|"assistant", "content": "..."}``).

Designed to be reusable across projects — no agent-specific logic.
...
def get_pair_count(history: list[dict]) -> int:
    """Return the number of user/assistant exchange pairs in history.
    ...
    -------
    int
        Number of complete pairs (each pair = 1 user + 1 assistant).
    """
...
def append_exchange(
    ...
    This is a pure function — it returns a new list and does not write
    to disk.  Call ``save_memory()`` afterwards to persist.
